                ...
                <div class="row">
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div>
                                    <h5><strong>Livrat catre: <?php echo $firstName; echo " "; echo $lastName ?></strong></h5>
                                </div>
                                <div>
                                    <h6> Numar de telefon: <?php echo $phone ?></h6>
                                </div>
                                <div>
                                    <h6> La adresa: <?php echo $address; echo", "; echo $city ?></h6>
                                </div>
                            </div>
                        </div>
                        ...
                        $query3 = mysqli_query($conn, "SELECT id_order FROM order_deliveries WHERE id_deliveries='$deliveriesId'");
                        if (mysqli_num_rows($query3) > 0) {
                            while ($row3 = mysqli_fetch_array($query3)) {
                                $orderId = $row3[0];
                                $query4 = mysqli_query($conn, "SELECT * FROM product_order WHERE id='$orderId'");
                                if (mysqli_num_rows($query4) > 0) {
                                    $row4 = mysqli_fetch_array($query4);
                                    $productId = $row4[1];
                                    $quantity = $row4[4];
                                    $query5 = mysqli_query($conn, "SELECT * FROM product WHERE id='$productId'");
                                    if (mysqli_num_rows($query5) > 0) {
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-8">
                                                ...
                                                <div>Numar de bucati comandate: <?php echo $quantity ?></div>
                                                <div></div>
                                            </div>
                                        </div>
                                        <br>
